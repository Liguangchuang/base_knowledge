...
def createTree(dataSet, featNames, min_gain):
    ...
    myTree = {bestFeat: {}}  #根据“最佳特征”，建立分支节点

    best_feat_Values_set = set([example[bestFeat_idx] for example in dataSet])

    for value in best_feat_Values_set:  #对于最佳特征的，每种取值
        sub_dataset = splitDataSet(dataSet, bestFeat_idx, value)   #根据每种取值，划分出子数据集
        sub_featNames = featNames[0: bestFeat_idx] + featNames[bestFeat_idx+1: ]

        # 为每个取值生成一个分支；并且分支下面的子数据集也做同样的操作（选择最佳特征并进行划分，或者输出类别标签）
        myTree[bestFeat][value] = createTree(sub_dataset, sub_featNames, min_gain=0.1)
    return myTree
